...
const fs = __importStar(__webpack_require__(3)); // Use promises for async file operations
// Import local modules
const simple_vector_1 = __webpack_require__(4); // Import our simple vector manager
const c_parser_1 = __webpack_require__(133); // Import our advanced C parser
// Import LangChain and LangGraph components
const openai_1 = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@langchain/openai'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
const output_parsers_1 = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@langchain/core/output_parsers'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
const prompts_1 = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@langchain/core/prompts'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
const langgraph_1 = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@langchain/langgraph'"); e.code = 'MODULE_NOT_FOUND'; throw e; }())); // Import START constant explicitly
const runnables_1 = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@langchain/core/runnables'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
// Add LangSmith imports
const langsmith_1 = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module 'langsmith'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
const callbacks_1 = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module 'langchain/callbacks'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
// Define the graph nodes
/**
 * Node: Retrieves context (similar functions) from the vector database.
 */
async function retrieveContext(state, vectorManager) {
    ...
    if (!vectorManager || !vectorManager.isReady()) {
        ...
        return {
            similarFunctionsCode: "// Vector database not available. Generating tests without similar function context.",
        };
    }
    try {
        // Search for functions similar to the target function's code
        const similarFunctions = await vectorManager.searchSimilarFunctions(state.functionCode, 3); // Limit to 3
...
async function generateTests(state) {
    ...
    const openaiApiKey = config.get("openaiApiKey") || process.env.OPENAI_API_KEY;
    if (!openaiApiKey) {
        console.error("OpenAI API key is not configured in either VS Code settings or environment variables.");
        return {
            errorMessage: "OpenAI API key is not configured in either VS Code settings or environment variables.",
        };
    }
    ...
    **Generated Unit Test Code:**
    `);
        ...
        console.log(`[${new Date().toISOString()}] === Invoking LLM for test generation ===`);
        ...
        vscode.window.showInformationMessage(`Generating tests for ${state.functionName} with GPT-4...`);
        const generatedCode = await testGeneratorChain.invoke({
            filePath: state.filePath,
            functionName: state.functionName,
            functionCode: state.functionCode,
            similarFunctionsCode: state.similarFunctionsCode || "// No context provided",
        });
        const duration = ((Date.now() - startTime) / 1000).toFixed(1);
        console.log(`[${new Date().toISOString()}] === LLM generation complete in ${duration}s ===`);
        console.log(`Generated ${generatedCode.length} characters of test code`);
        vscode.window.showInformationMessage(`Test generation complete in ${duration} seconds.`);
        return { generatedTestCode: generatedCode };
    }
    catch (error) {
        console.error(`[${new Date().toISOString()}] Error generating tests with LLM:`, error);
 ...
 */
async function findRelatedCFiles(baseFilePath) {
    try {
        ...
        const includePattern = new vscode.RelativePattern(workspaceFolders[0].uri.fsPath, `**/${baseFileName}.h`);
        ...
        const uniqueFilePaths = [...new Set(files.map((f) => f.fsPath))].filter((p) => p !== baseFilePath);
        // Read file contents
...
function activate(context) {
    ...
    // --- Initialization ---
    let vectorManager = null; // Initialize as null
    let cParser = null; // Initialize C parser
    let vectorDBAvailable = false; // Track if vector DB is available
    // Define the command handles outside any try/catch blocks
    let helloWorldDisposable;
    let printVectorsDisposable;
    let generateUnitTestDisposable;
    let configureDisposable;
    ...
    if (vectorDBDisabled) {
        ...
        vscode.window.showInformationMessage("Running in LLM-only mode (Vector DB disabled in settings)");
    }
    else {
        try {
            // Initialize the simple vector manager
            console.log("Using SimpleVectorManager for vector embeddings");
            vectorManager = new simple_vector_1.SimpleVectorManager();
            ...
            vectorManager
                ...
                if (initialized) {
                    console.log("SimpleVectorManager initialization successful (async).");
                }
                else {
                    console.error("SimpleVectorManager initialization failed (async).");
                    vscode.window.showWarningMessage("Vector database not available. The extension will work in reduced functionality mode without RAG context.");
                }
            })
                .catch((error) => {
                console.error("Error during async SimpleVectorManager initialization:", error);
                vscode.window.showWarningMessage("Vector database not available. The extension will work in reduced functionality mode without RAG context.");
            ...
            vscode.window.showWarningMessage(`Unable to connect to vector database: ${error.message}. The extension will work in reduced functionality mode without RAG context.`);
            // vectorManager remains null
        }
    }
    // Initialize C parser - do this in parallel with vector DB initialization
    cParser = new c_parser_1.CParser();
    cParser
        .initialize()
        .then((initialized) => {
        if (initialized) {
            ...
            console.warn("C parser initialization failed, will use fallback regex parser.");
        }
    })
        .catch((error) => {
    ...
    const generateTestsNode = new runnables_1.RunnableLambda({
        func: generateTests,
    });
    ...
    try {
        ...
        workflow.addEdge(langgraph_1.START, RETRIEVE_CONTEXT);
        workflow.addEdge(RETRIEVE_CONTEXT, GENERATE_TESTS);
        workflow.addEdge(GENERATE_TESTS, langgraph_1.END);
        // Compile the graph
        ...
        printVectorsDisposable = vscode.commands.registerCommand("rag-unit-testing.printVectorEmbeddings", async () => {
            // Check if manager is initialized before proceeding
            if (!vectorManager || !(await vectorManager.initialize())) {
                vscode.window.showErrorMessage("Vector DB is not available or failed to initialize. Cannot print embeddings.");
                return;
            }
            try {
                ...
                vscode.window.showInformationMessage(`Vector embeddings search for "${functionName}" initiated. Check Debug Console (Ctrl+Shift+Y).`);
            }
            catch (error) {
                console.error("Error in printVectorEmbeddings command:", error);
                vscode.window.showErrorMessage(`Failed to print vector embeddings: ${error.message}`);
            }
        });
        // Command to generate unit tests using LangGraph
        generateUnitTestDisposable = vscode.commands.registerCommand("rag-unit-testing.generateUnitTest", async (uri) => {
            ...
            if (!targetUri) {
                vscode.window.showErrorMessage("No file selected or active editor found. Please right-click a C file or open it.");
                return;
            }
            // Ensure it's a C file (basic check)
            if (!targetUri.fsPath.match(/\.(c|h)$/i)) {
                vscode.window.showWarningMessage("Please select a C source file (.c or .h).");
                return;
            }
            ...
            if (!app) {
                vscode.window.showErrorMessage("LangGraph application failed to compile. Cannot generate tests.");
                return;
            }
            try {
                ...
                if (vectorDBAvailable && !vectorDBDisabled) {
                    ...
                    console.log("Skipping vector DB context storage (DB not available or disabled)");
                }
                ...
                if (!functionName || functionName === "main") {
                    functionName = await vscode.window.showInputBox({
                        prompt: "Could not auto-detect function. Enter the function name to test:",
                        placeHolder: "e.g., calculate_sum",
                        value: functionName && functionName !== "main" ? functionName : "", // Pre-fill if partially detected
                    });
                }
                if (!functionName) {
                    vscode.window.showErrorMessage("No function name provided. Aborting test generation.");
                    return;
                }
                ...
                if (cParser && (await cParser.initialize())) {
                    ...
                    targetFunction = parsedFunctions.find((f) => f.functionName === functionName);
                }
                ...
                let functionCode = "";
                if (!targetFunction) {
                    console.warn(`Function '${functionName}' not found in parsed content of ${path.basename(filePath)}.`);
                    vscode.window.showWarningMessage(`Function '${functionName}' could not be found automatically. Continuing with user input.`);
                        ...
                        for (const relatedFile of relatedFiles) {
                            try {
                                ...
                                if (vectorManager &&
                                    ...
                                    !vectorDBDisabled) {
                                    await vectorManager.storeFileContext(relatedFile.path, relatedFile.content);
                                }
                                ...
                                console.warn(`Error processing related file ${relatedFile.path}:`, err);
                            }
                        }
                        console.log(`Added ${additionalContext.length} chars of context from related files`);
                    });
                }
                ...
                await vscode.window.withProgress({
                    ...
                    try {
                        ...
                        const testCode = apiResult?.generatedTestCode;
                        // Check if we have valid test code
                        if (!testCode || typeof testCode !== "string") {
                            throw new Error("Graph execution didn't produce valid test code output");
                        }
                        ...
                        const originalFilename = path.basename(targetUri.fsPath, path.extname(targetUri.fsPath));
                        const testFileName = `test${originalFilename}.c`;
                        const testFileUri = vscode.Uri.joinPath(targetUri, "..", testFileName);
                        ...
                        vscode.window.showInformationMessage(`Generated unit test file: ${testFileName}`);
                    }
                    catch (error) {
                ...
                if (error.message !== "Cancelled") {
                    const message = error.message.startsWith("Graph execution failed:") ||
                        error.message.includes("LLM generation failed")
                        ? error.message // Show specific graph/LLM errors directly
                        : `Error generating unit test: ${error.message || "Unknown error"}`;
        ...
        configureDisposable = vscode.commands.registerCommand("rag-unit-testing.configure", async () => {
            ...
            if (selectedOption === "Open Settings in Editor") {
                vscode.commands.executeCommand("workbench.action.openSettings", "rag-unit-testing");
                return;
            }
        ...
        vscode.window.showErrorMessage(`Failed to set up LangGraph workflow: ${graphError.message}`);
        // Continue with command registration anyway, so basic commands still work
        // Register just the basic commands
        helloWorldDisposable = vscode.commands.registerCommand("rag-unit-testing.helloWorld", () => {
            vscode.window.showInformationMessage("Hello World from RAG Unit Testing!");
        });
        const printVectorsDisposable = vscode.commands.registerCommand("rag-unit-testing.printVectorEmbeddings", async () => {
            vscode.window.showErrorMessage("Feature unavailable due to graph initialization failure.");
        });
        const generateUnitTestDisposable = vscode.commands.registerCommand("rag-unit-testing.generateUnitTest", async () => {
            vscode.window.showErrorMessage("Cannot generate tests due to LangGraph initialization failure.");
        });
        // Add these fallback disposables to context
        context.subscriptions.push(helloWorldDisposable, printVectorsDisposable, generateUnitTestDisposable);
        return; // Exit activation
    }
    // Add disposables to context subscriptions
    context.subscriptions.push(helloWorldDisposable, printVectorsDisposable, generateUnitTestDisposable, configureDisposable);
}
...
function parseCFunctions(fileContent) {
    ...
    if (functions.length === 0) {
        ...
        while ((match = relaxedFunctionRegex.exec(contentWithoutComments)) !== null) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.SimpleVectorManager = void 0;
...
const dotenv = __importStar(__webpack_require__(130));
...
function parseCFunctions(fileContent) {
    ...
    const functionRegex = /^([\w\s\*]+?)\s+(\w+)\s*\(([^)]*)\)\s*(?:;|\{([\s\S]*?)(?:^|\n)\s*\})/gm;
    let match;
    while ((match = functionRegex.exec(contentWithoutComments)) !== null) {
        // Improved filtering to avoid matching struct initializations or other constructs
        if (match[1].includes(";") ||
...
class SimpleVectorManager {
    ...
    functionStore = [];
    cacheDir;
    embeddingCache = {}; // In-memory cache
    pendingEmbeddings = new Map(); // To prevent duplicate requests
    batchQueue = [];
    batchTimer = null;
    constructor() {
        ...
        console.log(`- OPENAI_API_KEY source: ${config.get("openaiApiKey") ? "VS Code settings" : "Environment variable"}`);
        ...
        console.log(`- OPENAI_API_KEY: ${openaiApiKey
            ? `✓ (set, length: ${openaiApiKey.length}, starts with: ${openaiApiKey.substring(0, 10)}...)`
            : "✗ (missing)"}`);
        if (!openaiApiKey) {
        ...
        try {
            ...
            if (fs.existsSync(cacheFile)) {
                try {
                    ...
                    console.log(`Loaded ${this.functionStore.length} cached function embeddings`);
                }
                catch (err) {
            ...
            vscode.window.showErrorMessage(`Failed to initialize vector storage: ${error.message}`);
            return false;
        }
    }
    /**
    ...
    loadEmbeddingCache() {
        try {
            ...
            if (fs.existsSync(cacheFile)) {
                ...
                console.log(`Loaded embedding cache with ${Object.keys(this.embeddingCache).length} entries (removed ${expiredCount} expired entries)`);
            }
            else {
    ...
    saveEmbeddingCache() {
        try {
            ...
            console.log(`Saved embedding cache with ${Object.keys(this.embeddingCache).length} entries`);
        }
        catch (error) {
    ...
    async generateEmbedding(text) {
        ...
        const textHash = (0, crypto_1.createHash)("sha256").update(text).digest("hex");
        ...
        if (this.embeddingCache[textHash] &&
    ...
    async processBatch() {
        ...
        try {
            ...
            response.data.forEach((result, index) => {
                ...
                const textHash = (0, crypto_1.createHash)("sha256").update(item.text).digest("hex");
                // Update cache
            ...
            batch.forEach((item) => {
                item.reject(new Error(`Failed to generate embedding: ${error.message}`));
            });
    ...
    async saveCache() {
        const cacheFile = path.join(this.cacheDir, "function-embeddings.json");
        try {
            await fs.promises.writeFile(cacheFile, JSON.stringify(this.functionStore), "utf8");
            console.log(`Saved ${this.functionStore.length} function embeddings to cache`);
        }
        catch (err) {
     ...
     */
    async storeFileContext(filePath, fileContent) {
        if (!this.isInitialized) {
            console.log("Initializing SimpleVectorManager before storing context...");
            const initialized = await this.initialize();
            if (!initialized) {
                vscode.window.showErrorMessage("Cannot store file context: Vector storage initialization failed.");
                return;
            }
        }
        try {
            ...
            if (functions.length === 0) {
                ...
                vscode.window.showInformationMessage("No functions found to store from this file.");
                return;
            }
            console.log(`Found ${functions.length} functions in ${filePath} to store:`);
            // Remove existing functions from this file path
            this.functionStore = this.functionStore.filter((f) => f.filePath !== filePath);
            ...
            vscode.window.showInformationMessage(`Successfully stored ${functions.length} functions from ${path.basename(filePath)}.`);
        }
        catch (error) {
            console.error("Error storing file context:", error);
            vscode.window.showErrorMessage(`Failed to store file context: ${error.message}`);
        }
    }
    /**
     ...
     */
    async searchSimilarFunctions(queryText, limit = 5) {
        if (!this.isInitialized) {
            console.warn("Attempted search before SimpleVectorManager was initialized.");
            const initialized = await this.initialize();
            if (!initialized) {
        ...
        try {
            ...
            console.log(`Searching for functions similar to query text (length: ${queryText.length})`);
            ...
            results.forEach((func, i) => {
                console.log(` - ${i + 1}. ${func.functionName} (Distance: ${func.distance.toFixed(4)})`);
            });
            return results;
        }
        catch (error) {
            console.error("Error searching similar functions:", error);
            vscode.window.showErrorMessage(`Failed to search for similar functions: ${error.message}`);
            return [];
        }
    }
    /**
     ...
     */
    async printVectorEmbeddingsForFunctions(functionName) {
        if (!this.isInitialized) {
        ...
        try {
            ...
            const matchingFunctions = this.functionStore.filter((func) => func.functionName.includes(functionName));
            ...
            matchingFunctions.forEach((func) => {
                ...
                console.log(`Last Updated: ${func.lastUpdated
                ...
                if (func.embedding && func.embedding.length > 0) {
                    ...
                    console.log(`First 10 dimensions: [${func.embedding
                        .slice(0, 10)
                        .map((v) => v.toFixed(4))
                        .join(", ")}...]`);
                }
            ...
            vscode.window.showErrorMessage(`Failed to fetch vectors: ${error.message}`);
        }
    }
    /**
...
exports.SimpleVectorManager = SimpleVectorManager;
...
const qs = __importStar(__webpack_require__(6));
...
const images_1 = __webpack_require__(116);
const models_1 = __webpack_require__(117);
const moderations_1 = __webpack_require__(118);
const audio_1 = __webpack_require__(84);
const beta_1 = __webpack_require__(89);
const chat_1 = __webpack_require__(78);
const fine_tuning_1 = __webpack_require__(113);
const responses_1 = __webpack_require__(119);
const uploads_1 = __webpack_require__(123);
const vector_stores_1 = __webpack_require__(125);
...
const formats_1 = __webpack_require__(7);
const formats = {
...
var stringify_1 = __webpack_require__(8);
 ...
 */
const shims = __webpack_require__(14);
const auto = __webpack_require__(15);
exports.init = () => {
 ...
 * Disclaimer: modules in _shims aren't intended to be imported by SDK users.
 */
const nf = __importStar(__webpack_require__(17));
const fd = __importStar(__webpack_require__(32));
const agentkeepalive_1 = __importDefault(__webpack_require__(43));
const abort_controller_1 = __webpack_require__(49);
const node_fs_1 = __webpack_require__(51);
const form_data_encoder_1 = __webpack_require__(52);
const node_stream_1 = __webpack_require__(63);
const MultipartBody_1 = __webpack_require__(64);
const web_1 = __webpack_require__(65);
let fileFromPathWarned = false;
async function fileFromPath(path, ...args) {
    // this import fails in environments that don't handle export maps correctly, like old versions of Jest
    const { fileFromPath: _fileFromPath } = await Promise.resolve().then(() => __importStar(__webpack_require__(66)));
...
const File_1 = __webpack_require__(35);
const isFile_1 = __webpack_require__(40);
const isBlob_1 = __webpack_require__(41);
const isFunction_1 = __webpack_require__(38);
const deprecateConstructorEntries_1 = __webpack_require__(42);
class FormData {
    constructor(entries) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.File = void 0;
const Blob_1 = __webpack_require__(36);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.Blob = void 0;
const web_streams_polyfill_1 = __webpack_require__(37);
const isFunction_1 = __webpack_require__(38);
const blobHelpers_1 = __webpack_require__(39);
class Blob {
...
const HttpAgent = __webpack_require__(44);
module.exports = HttpAgent;
module.exports.HttpAgent = HttpAgent;
module.exports.HttpsAgent = __webpack_require__(48);
module.exports.constants = __webpack_require__(47);
...
const OriginalAgent = (__webpack_require__(19).Agent);
const ms = __webpack_require__(45);
const debug = (__webpack_require__(34).debuglog)('agentkeepalive');
const {
  INIT_SOCKET,
  CURRENT_ID,
  CREATE_ID,
...
const OriginalHttpsAgent = (__webpack_require__(30).Agent);
const HttpAgent = __webpack_require__(44);
const {
  INIT_SOCKET,
...
var eventTargetShim = __webpack_require__(50);
...
const createBoundary_1 = __importDefault(__webpack_require__(54));
const isPlainObject_1 = __importDefault(__webpack_require__(55));
const normalizeValue_1 = __importDefault(__webpack_require__(56));
const escapeName_1 = __importDefault(__webpack_require__(57));
const isFileLike_1 = __webpack_require__(58);
const isFormData_1 = __webpack_require__(60);
...
const isFunction_1 = __importDefault(__webpack_require__(59));
...
var files_1 = __webpack_require__(112);
...
const resource_1 = __webpack_require__(79);
const JobsAPI = __importStar(__webpack_require__(114));
const jobs_1 = __webpack_require__(114);
...
const resource_1 = __webpack_require__(79);
const core_1 = __webpack_require__(10);
const CheckpointsAPI = __importStar(__webpack_require__(115));
const checkpoints_1 = __webpack_require__(115);
const pagination_1 = __webpack_require__(75);
...
const ResponsesParser_1 = __webpack_require__(120);
const core_1 = __webpack_require__(10);
const resource_1 = __webpack_require__(79);
const InputItemsAPI = __importStar(__webpack_require__(121));
const input_items_1 = __webpack_require__(121);
const ResponseStream_1 = __webpack_require__(122);
const pagination_1 = __webpack_require__(75);
...
const resource_1 = __webpack_require__(79);
const PartsAPI = __importStar(__webpack_require__(124));
const parts_1 = __webpack_require__(124);
...
const resource_1 = __webpack_require__(79);
const core_1 = __webpack_require__(10);
const FileBatchesAPI = __importStar(__webpack_require__(126));
const file_batches_1 = __webpack_require__(126);
const FilesAPI = __importStar(__webpack_require__(128));
const files_1 = __webpack_require__(128);
const pagination_1 = __webpack_require__(75);
...
const core_1 = __webpack_require__(10);
const core_2 = __webpack_require__(10);
const Util_1 = __webpack_require__(127);
const files_1 = __webpack_require__(128);
...
const packageJson = __webpack_require__(132)
...
class CParser {
     ...
     */
    parseFunctions(fileContent, filePath) {
        try {
            const functions = this.advancedParseFunctions(fileContent);
            console.log(`Advanced parser found ${functions.length} functions in ${filePath || "code"}`);
            return functions;
        }
        catch (error) {
     ...
     */
    advancedParseFunctions(fileContent) {
     ...
     */
    extractFunctionsWithPattern1(code, functions) {
     ...
     */
    extractFunctionsWithPattern2(code, functions) {
     ...
     */
    extractFunctionsWithPattern3(code, functions) {
     ...
     */
    shouldSkipMatch(returnType, body) {
        if (!body)
            return true;
     ...
     */
    fallbackParseFunctions(fileContent) {
